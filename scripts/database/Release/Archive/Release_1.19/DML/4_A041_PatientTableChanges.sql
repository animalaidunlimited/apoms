...
BEGIN
  ...
  ALTER TABLE `AAU`.`Patient` 
  ADD COLUMN `SameAsEmergencyCaseId` INT NULL DEFAULT NULL AFTER `PatientCallOutcomeId`;
  
  IF (SELECT COUNT(1) FROM AAU.Patient WHERE PatientCallOutcomeId IS NOT NULL) = 0 THEN
  
  UPDATE AAU.Patient p
  INNER JOIN AAU.EmergencyCase ec ON ec.EmergencyCaseId = p.EmergencyCaseId
  SET p.PatientCallOutcomeId = ec.CallOutcomeId, p.SameAsEmergencyCaseId = ec.SameAsEmergencyCaseId;

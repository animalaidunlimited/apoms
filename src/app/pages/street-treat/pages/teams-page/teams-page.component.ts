...
export class TeamsPageComponent implements OnInit {
   // teamList!: TeamDetails[];
    ...
    getrefreshTableData(){
      this.teamSubsciption = this.teamDetailService.getAllTeams().subscribe((teamListData: TeamDetails[])=>{
    ...
    Submit(){
      ...
      {
        this.teamDetailService.saveTeam(this.teamDetails.value).then((res:any)=>{
          if(res.vSuccess){
            this.snackBar.successSnackBar("Team saved succesfully!",'Ok');
            this.refreshTable();
            this.resetForm();
          }
          else {
    ...
    selectRow(selectedTeam:TeamDetails)
    {
      try{
        ...
        this.disabledDeleteBtn = (this.teamDetails.get('TeamId')?.value === null) ? true : false;
      }
      catch(err)
      {
        console.log(err);
    ...
    deleteTeam(){
     //let teamDeleteId = this.teamDetails.get('TeamId')?.value;
